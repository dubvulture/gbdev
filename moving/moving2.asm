...
CICLO:
	ld	a,[rSCX]			; save for future comparison (if sprite has not moved, do not redraw sprites for animation)
	ld	b,a
	ld	a,[rSCY]
	ld	c,a
...
wait:
	...
	ld	a,[rSCX]
	xor b
	ld	b,a					; if b=0 -> prev_SCX = act_SCX

	ld	a,[rSCY]
	xor	c					; if c=0 -> prev_SCY = act_SCY
	
	or	b
	jr	z,CICLO
	nop
	
; Now sprite = movement. Let's change that to still
